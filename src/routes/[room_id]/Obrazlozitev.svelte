<script lang="ts">
  ...
  const realizacija = (realizacija: Realizacija, ime: string, vrednost: number, napoved: NapovedBonusa) => {
    // let napovedano = '';
    // if (napoved === NapovedBonusa.Napovedan) {
    //   vrednost *= 2;
    //   napovedano = ' (napovedano)';
    // }
    // if (realizacija === Realizacija.Narejena) {
    //   return `${ime}: +${vrednost}${napovedano}`;
    // } else if (napoved !== NapovedBonusa.Napovedan) {
    //   return `${ime}: -${vrednost}${napovedano}`;
    // } else {
    //   return ``;
    // }
    ...
    switch (realizacija) {
      case Realizacija.Narejena:
        return je_napovedano ? `${ime}: +${vrednost} (x2 napovedano)` : `${ime}: +${vrednost}`;
      case Realizacija.Brez:
        return je_napovedano ? `${ime}: -${vrednost} (x2 napovedano)` : ``;
      case Realizacija.Izgubljena:
        return je_napovedano ? -tocke * 4 : `${ime}: -${vrednost}`;
    }
  };
</script>
  ...
  {#if round.round !== undefined}
    ...
    {:else if round_type === NewRoundType.Osnovno}
      ...
      {#if 'trula' in round.round}
        <br />
        {realizacija(round.round.trula, 'Trula', 10, napoved.trula)}
      {/if}
      {#if 'kralji' in round.round}
        <br />
        {realizacija(round.round.kralji, 'Kralji', 10, napoved.kralji)}
      {/if}
      {#if 'kralj_ultimo' in round.round}
        <br />
        {realizacija(round.round.kralj_ultimo, 'Kralj ultimo', 10, napoved.kralj_ultimo)}
      {/if}
      {#if 'pagat_ultimo' in round.round}
        <br />
        {realizacija(round.round.pagat_ultimo, 'Pagat ultimo', 25, napoved.pagat_ultimo)}
      {/if}
      <hr />
  ...
  <table class="divide-y divide-primary-700 w-full">
    ...
    <tr>
      ...
      {#each round.points_change as score}
        <td class="text-right px-2">{score >= 0 ? '+' : ''}{score}</td>
      {/each}
    </tr>
    <tr>
      <td>Radelci</td>
      {#each round.radelc_change as radlci}
        <td class="text-right px-2">{radlci >= 0 ? '+' : ''}{radlci}</td>
      {/each}
    </tr>
  </table>
</div>
